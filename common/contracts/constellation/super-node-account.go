...
// The number of seconds that a subnode operator must wait before they can withdraw their locked ETH
func (c *SuperNodeAccount) LockupTime(mc *batch.MultiCaller, out **big.Int) {
	eth.AddCallToMulticaller(mc, c.contract, out, "lockUpTime")
}
...
func (c *SuperNodeAccount) GetMinipoolCount(mc *batch.MultiCaller, out **big.Int) {
	eth.AddCallToMulticaller(mc, c.contract, out, "minipoolCount")
}
...
func (c *SuperNodeAccount) CreateMinipool(
	...
	nodeSetSignatureTime time.Time,
	nodeSetSignature []byte, opts *bind.TransactOpts,
) (*eth.TransactionInfo, error) {
	timestamp := nodeSetSignatureTime.UTC().Unix()
	...
	cfg := createMinipoolConfig{
		...
		SignatureGenesisTime:    timestampBig,
