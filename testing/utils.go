...
func (m *ConstellationTestManager) MineTx(t *testing.T, txInfo *eth.TransactionInfo, opts *bind.TransactOpts, logMessage string) {
	// Services
	txMgr := m.GetServiceProvider().GetTransactionManager()
...
func (m *ConstellationTestManager) MineTxViaHyperdrive(t *testing.T, txInfo *eth.TransactionInfo, logMessage string) {
	// Submit the tx
	hd := m.HyperdriveTestManager.GetApiClient()
	maxFee := eth.GweiToWei(1)
	maxPrioFee := eth.GweiToWei(0.1)
	submission, _ := eth.CreateTxSubmissionFromInfo(txInfo, nil)
	txResponse, err := hd.Tx.SubmitTx(submission, nil, maxFee, maxPrioFee)
	require.NoError(t, err)
